    ...
    var body: some View {
        ZStack{
            ...
            VStack{
                ...
                Spacer()
                ToolBarView(selectedTab: $selectedTab, strokeWidth: $strokeWidth, startInpainting: $startInpainting, didChange: $didChange, imageCropperShow: $cropperShown, selectedOption: $selectedEditOption, fetchOptionPercentValue: fetchOptionPercentValue, ratio: $ratio)
//                    .frame(height: 100)
        ...
        .fullScreenCover(isPresented: $cropperShown){
            ImageCropperView(isShowing: $cropperShown, image: $vm.targetImage, cropShapeType: .rect, presetFixedRatioType: .canUseMultiplePresetFixedRatio(), ratio: $ratio)
                .ignoresSafeArea(edges: .bottom)
            ToolBarView(selectedTab: $selectedTab, strokeWidth: $strokeWidth, startInpainting: $startInpainting, didChange: $didChange, imageCropperShow: $cropperShown, selectedOption: $selectedEditOption, fetchOptionPercentValue: fetchOptionPercentValue, ratio: $ratio)
        }
        .onDisappear {
